export interface BnccSampleQuestion {
  question: string;
  options: string[];
  correctIndex: number;
  pointsReward: number;
...
export interface BnccSkill {
  ...
  stage: string;
  description: string;
  sampleQuestions: BnccSampleQuestion[];
}

const BNCC_OFFICIAL_SKILLS: BnccSkill[] = [
  {
    code: 'EM13CHS202',
...
export class BnccService {
  ...
  static getSkillByCode(code: string) {
    if (!code) return null;
    return BNCC_OFFICIAL_SKILLS.find((s) => s.code.toUpperCase() === code.toUpperCase()) || null;
  }

  static generateQuiz({ bnccCode, subject }: { bnccCode?: string; subject?: string }) {
    const skill = (bnccCode ? this.getSkillByCode(bnccCode) : null) || BNCC_OFFICIAL_SKILLS[0];
