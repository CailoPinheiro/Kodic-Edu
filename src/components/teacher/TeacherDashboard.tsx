...
import { Activity, BrainCircuit, BarChart3, Shield, Megaphone, Plus, CheckSquare, CheckCircle2 } from 'lucide-react';
...
export function TeacherDashboard({ currentClass, onDataChange }: TeacherDashboardProps) {
  ...
  return (
    <div className="space-y-4">
      <div className={`px-2 py-3 flex gap-2 overflow-x-auto scrollbar-hide border-b ${isDarkMode ? 'border-violet-900/50' : 'border-violet-100'} relative z-10 bg-inherit`}>
        {[
          { id: 'visao_geral', label: 'Visão Geral', icon: Activity },
          { id: 'bncc', label: 'Assistente BNCC', icon: BrainCircuit },
          ...
          <button
            ...
            className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold flex items-center gap-1.5 transition-all ${
              teacherTab === tab.id
                ? `${t.primaryGrad} text-white shadow-md`
          ...
          <>
            ...
            <div className={`${t.card} rounded-3xl p-5 relative overflow-hidden transition-colors`}>
              <h2 className={`${t.textMain} font-bold text-sm mb-1`}>
                Meta Coletiva da Turma ({currentClass?.code || 'GEO-2026'})
              </h2>
