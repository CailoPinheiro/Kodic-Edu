...
const MOCK_SCHOOLS = [
  { name: 'Escola Estadual Profa. Maria Heloísa', city: 'São Paulo' },
  { name: 'Colégio Municipal Dom Pedro II', city: 'Rio de Janeiro' },
  { name: 'Escola Técnica Estadual Ayrton Senna', city: 'Belo Horizonte' },
  { name: 'Instituto Educacional Novo Horizonte', city: 'Curitiba' }
];

function ensureSchoolsSeeded(): void {
  const insertSchool = db.prepare('INSERT OR IGNORE INTO schools (name, city) VALUES (?, ?)');
  for (const school of MOCK_SCHOOLS) {
    insertSchool.run(school.name, school.city);
  }
}
